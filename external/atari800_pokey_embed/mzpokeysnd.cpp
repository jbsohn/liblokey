...
#include "atari.h"
#endif
#include "mzpokeysnd.hpp"
#include "pokeysnd.hpp"
#include "remez.hpp"
#include "antic.h"
#include "gtia.h"
...
typedef int (*readout_t)(stPokeyState* ps);
typedef void (*event_t)(stPokeyState* ps, int p5v, int p4v, int p917v);
...
static void bump_qe_subticks(PokeyState* ps, int subticks) {
    ...
    static constexpr int tickoverflowlimit = 1000000000;
    ps->curtick += subticks;
...
static void build_poly4() {
    unsigned char poly4 = 1;

    for (int& i : poly4tbl) {
        i = ~poly4;
        const unsigned char c = poly4 >> 2 & 1 ^ poly4 >> 3 & 1;
        poly4 = (poly4 << 1 & 15) + c;
    }
}

static void build_poly5() {
    unsigned char poly5 = 1;

    for (unsigned char i = 0; i < 31; i++) {
        poly5tbl[i] = ~poly5; /* Inversion! Attention! */
        const unsigned char c = (poly5 >> 2 ^ poly5 >> 4) & 1;
        poly5 = (poly5 << 1 & 31) + c;
    }
}

static void build_poly17() {
    unsigned int poly17 = 1;

    for (unsigned char& i : poly17tbl) {
        i = static_cast<unsigned char>(poly17);
        unsigned int c = ((poly17 >> 11) ^ (poly17 >> 16)) & 1;
        poly17 = ((poly17 << 1) & 131071) + c;
    }
}

static void build_poly9() {
    unsigned int poly9 = 1;

    for (unsigned int i = 0; i < 511; i++) {
        poly9tbl[i] = static_cast<unsigned char>(poly9);
        const unsigned int c = (poly9 >> 3 ^ poly9 >> 8) & 1;
        poly9 = (poly9 << 1 & 511) + c;
    }
}

static void advance_polies(PokeyState* ps, const int tacts) {
...
static void event0_pure(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    ps->c0t2 = !ps->c0t2;
    ps->c0t1 = p5v;
}

static void event0_p5(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    if (ps->c0t1)
        ps->c0t2 = !ps->c0t2;
    ps->c0t1 = p5v;
}

static void event0_p4(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    ps->c0t2 = p4v;
    ps->c0t1 = p5v;
}

static void event0_p917(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    ps->c0t2 = p917v;
    ps->c0t1 = p5v;
}

static void event0_p4_p5(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    if (ps->c0t1)
        ps->c0t2 = p4v;
    ps->c0t1 = p5v;
}

static void event0_p917_p5(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    if (ps->c0t1)
        ps->c0t2 = p917v;
    ps->c0t1 = p5v;
}
...
static void event1_pure(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    ps->c1t2 = !ps->c1t2;
    ps->c1t1 = p5v;
}

static void event1_p5(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    if (ps->c1t1)
        ps->c1t2 = !ps->c1t2;
    ps->c1t1 = p5v;
}

static void event1_p4(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    ps->c1t2 = p4v;
    ps->c1t1 = p5v;
}

static void event1_p917(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    ps->c1t2 = p917v;
    ps->c1t1 = p5v;
}

static void event1_p4_p5(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    if (ps->c1t1) {
        ps->c1t2 = p4v;
    }
    ps->c1t1 = p5v;
...
static void event1_p917_p5(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    if (ps->c1t1) {
        ps->c1t2 = p917v;
    }
    ps->c1t1 = p5v;
...
static void event2_pure(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    ps->c2t2 = !ps->c2t2;
    ps->c2t1 = p5v;
...
static void event2_p5(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    if (ps->c2t1) {
        ps->c2t2 = !ps->c2t2;
    }
    ps->c2t1 = p5v;
...
static void event2_p4(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    ps->c2t2 = p4v;
    ps->c2t1 = p5v;
    /* high-pass clock for channel 0 */
    ps->c0t3 = ps->c0t2;
...
static void event2_p917(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    ps->c2t2 = p917v;
    ps->c2t1 = p5v;
    /* high-pass clock for channel 0 */
    ps->c0t3 = ps->c0t2;
...
static void event2_p4_p5(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    if (ps->c2t1) {
        ps->c2t2 = p4v;
    }
    ps->c2t1 = p5v;
...
static void event2_p917_p5(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    if (ps->c2t1)
        ps->c2t2 = p917v;
    ps->c2t1 = p5v;
    /* high-pass clock for channel 0 */
...
static void event3_pure(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    ps->c3t2 = !ps->c3t2;
    ps->c3t1 = p5v;
...
static void event3_p5(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    if (ps->c3t1) {
        ps->c3t2 = !ps->c3t2;
    }
    ps->c3t1 = p5v;
...
static void event3_p4(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    ps->c3t2 = p4v;
    ps->c3t1 = p5v;
    /* high-pass clock for channel 1 */
    ps->c1t3 = ps->c1t2;
...
static void event3_p917(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    ps->c3t2 = p917v;
    ps->c3t1 = p5v;
    /* high-pass clock for channel 1 */
    ps->c1t3 = ps->c1t2;
...
static void event3_p4_p5(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    if (ps->c3t1) {
        ps->c3t2 = p4v;
    }
    ps->c3t1 = p5v;
...
static void event3_p917_p5(PokeyState* ps, const int p5v, const int p4v, const int p917v) {
    if (ps->c3t1) {
        ps->c3t2 = p917v;
    }
    ps->c3t1 = p5v;
...
static void advance_ticks(PokeyState* ps, int ticks) {
    ...
    while (ticks > 0) {
        int tbe0 = ps->c0divpos;
        int tbe1 = ps->c1divpos;
        int tbe2 = ps->c2divpos;
        int tbe3 = ps->c3divpos;
...
static void Update_audctl(PokeyState* ps, const unsigned char val) {
    int new_divk;
    int recalc0 = 0;
    int recalc1 = 0;
    int recalc2 = 0;
    ...
    const int nc0_hf = (val & 0x40) != 0;
    const int nc2_hf = (val & 0x20) != 0;
    const int nc1_f0 = (val & 0x10) != 0;
    const int nc3_f2 = (val & 0x08) != 0;
    const int nc0sw4 = (val & 0x04) != 0;
    const int nc1sw4 = (val & 0x02) != 0;
    if (val & 0x01)
        new_divk = 114;
    else
        new_divk = 28;
...
/* SKCTL for two-tone mode */
static void Update_skctl(PokeyState* ps, const unsigned char val) {
    ps->skctl = val;
}
...
static void Update_c1stop(PokeyState* ps) {
    ...
    if (!ps->c1_f0 && (ps->c1vo || ps->vol1 == 0)) {
        ...
        ps->c1stop = 1;
        hfa = 1;
    } else if (!ps->c1sw4 && ps->c1sw3 && !ps->c1sw2 && ps->c1sw1 &&
               ps->c1divstart <= lim * 2 / 15) /* all poly4 signal is above Nyquist */
    {
        ps->c1stop = 1;
        hfa = 1;
    }
...
static void Update_c3stop(PokeyState* ps) {
    ...
    /* If channel 3 is a pure tone */
        ...
        ps->c3stop = 1;
        hfa = 1;
    } else if (ps->c3sw3 && !ps->c3sw2 && ps->c3sw1 && !ps->c1sw4 &&
               ps->c3divstart <= lim * 2 / 15) /* all poly4 signal is above Nyquist */
    {
        ps->c3stop = 1;
        hfa = 1;
    }
...
/*                                                                           */
/*****************************************************************************/
static void Update_pokey_sound_mz(const UWORD addr, const UBYTE val, const UBYTE chip, UBYTE gain) {
...
static void generate_sync(unsigned int num_ticks) {
    ...
    const UBYTE* buffer_end = POKEYSND_process_buffer + POKEYSND_process_buffer_length;
    unsigned int i;
