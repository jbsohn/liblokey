        ...
        case POKEY_OFFSET_RANDOM:
            if ((POKEY_SKCTL & 0x03) != 0) {
                ...
                if (POKEY_AUDCTL[0] & POKEY_POLY9)
                    ...
                    i %= POKEY_POLY17_SIZE;
                    const UBYTE* ptr = POKEY_poly17_lookup + (i >> 3);
                    i &= 7;
                    byte = static_cast<UBYTE>((ptr[0] >> i) + (ptr[1] << (8 - i)));
                }
            }
            break;
...
static int POKEY_siocheck() {
    return (((POKEY_AUDF[POKEY_CHAN3] == 0x28 || POKEY_AUDF[POKEY_CHAN3] == 0x10 || POKEY_AUDF[POKEY_CHAN3] == 0x08 ||
              POKEY_AUDF[POKEY_CHAN3] == 0x0a) &&
...
void POKEY_PutByte(UWORD addr, UBYTE byte) {
    ...
    switch (addr) {
        ...
        case POKEY_OFFSET_AUDF1:
            ...
            Update_Counter((POKEY_AUDCTL[0] & POKEY_CH1_CH2) ? ((1 << POKEY_CHAN2) | (1 << POKEY_CHAN1))
                                                             : (1 << POKEY_CHAN1));
        ...
        case POKEY_OFFSET_AUDF3:
            ...
            Update_Counter((POKEY_AUDCTL[0] & POKEY_CH3_CH4) ? ((1 << POKEY_CHAN4) | (1 << POKEY_CHAN3))
                                                             : (1 << POKEY_CHAN3));
        ...
        case POKEY_OFFSET_SEROUT:
            ...
            if ((POKEY_SKCTL & 0x08) == 0x00) {
                ...
                POKEY_DELAYED_SEROUT_IRQ =
                    312 * 50 * 10 * (POKEY_AUDF[POKEY_CHAN3] + POKEY_AUDF[POKEY_CHAN4] * 0x100) / 895000;
                /* safety check */
...
int POKEY_Initialise(int* argc, char* argv[]) {
    ...
    for (i = 0; i < 511; i++) {
        ...
        POKEY_poly9_lookup[i] = static_cast<UBYTE>(reg);
    }
    /* initialise poly17_lookup */
    reg = 0x1ffff;
    for (i = 0; i < 16385; i++) {
        reg = ((((reg >> 5) ^ reg) & 0xff) << 9) + (reg >> 8);
        POKEY_poly17_lookup[i] = static_cast<UBYTE>(reg >> 1);
    }
    ...
    if (chan_mask & (1 << POKEY_CHAN2)) {
        ...
        if (POKEY_AUDCTL[0] & POKEY_CH1_CH2) {
            if (POKEY_AUDCTL[0] & POKEY_CH1_179)
                ...
                POKEY_DivNMax[POKEY_CHAN2] =
                    (POKEY_AUDF[POKEY_CHAN2] * 256 + POKEY_AUDF[POKEY_CHAN1] + 1) * POKEY_Base_mult[0];
        } else
    ...
    if (chan_mask & (1 << POKEY_CHAN4)) {
        ...
        if (POKEY_AUDCTL[0] & POKEY_CH3_CH4) {
            if (POKEY_AUDCTL[0] & POKEY_CH3_179)
                ...
                POKEY_DivNMax[POKEY_CHAN4] =
                    (POKEY_AUDF[POKEY_CHAN4] * 256 + POKEY_AUDF[POKEY_CHAN3] + 1) * POKEY_Base_mult[0];
        } else
...
void POKEY_StateRead() {
    ...
    for (i = 0; i < 4; i++) {
        POKEY_PutByte(static_cast<UWORD>((POKEY_OFFSET_AUDF1 + i * 2)), POKEY_AUDF[i]);
        POKEY_PutByte(static_cast<UWORD>((POKEY_OFFSET_AUDC1 + i * 2)), POKEY_AUDC[i]);
    }
